...
function ChatBoxCoze() {
  const user_id = useStore((state) => state.userInfo?.id); // Lấy ra user id từ fetchUserInfo ở Zustand
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className={`chat-container flex flex-col ${fullscreen ? '!w-screen !h-screen !bottom-0 !left-0 !rounded-none' : ''}`}
        >
          ...
          <div className="flex-1 overflow-y-auto space-y-2 p-4 bg-white">
            {messages.map((msg, index) => (
              <div key={index} className={`my-2 px-4 flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div
                ...
                >
                  {msg.type === 'products' ? (
                    <div className="grid grid-cols-1 gap-4">
                      {msg.content.map((product, i) => (
                        <div key={i} className="border rounded-xl p-4 shadow bg-white">
                          <img src={product.images} className="w-full h-32 object-cover rounded-lg" />
                          ...
                          <div className="addToCart flex gap-4 items-center bg-[#f1f1f1] p-2 my-2 rounded-md">
                            <select
                              ...
                              onChange={(e) =>
                                handleChooseSize(e, product.productId, product.variations, {
                            ...
                            >
                              <IoBagAddSharp className="text-[20px]" />
                            </button>
                          </div>
                        </div>
                      ))}
                    ...
                    <button onClick={() => sendMessage(msg.content)}>{msg.content}</button>
                  ) : msg.type === 'loading' ? (
          ...
          <div className="flex gap-2 p-4 bg-white border-t border-[#ccc] rounded-b-xl">
            ...
            />
            <button className="text-white" onClick={() => sendMessage()} disabled={loading}>
